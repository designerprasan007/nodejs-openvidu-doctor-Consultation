...
function Session_name() {
	...
	session.on('streamCreated', function (event) {
		...
		var subscriber = session.subscribe(event.stream, 'screen_sh');
	...
	getToken(sessionId).then(token => {
		...
		session.connect(token)
			.then(() => {
				...
				publisher = OV.initPublisher('self_screen', {
					videoSource: 'screen', // The source of video. If undefined default video input
...
function updateLayout() {
	...
	var subscriberVideos = $('#screen_sh > video');
