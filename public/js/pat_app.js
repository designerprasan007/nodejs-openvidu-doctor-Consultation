...
function Session_name() {
	return room_name = $('#room_name').val();
	// console.log(sessionId + room_name);
	// console.log('above is room and session code');
	...
	session.on('streamCreated', function (event) {
		...
		subscriber.on('videoElementCreated', function (event) {
			numOfVideos++;
			if (numOfVideos == 2) {
				$('#calling_buttons').show();
				// alert('2 videos');
		  	    // $('#selfvideo').toggleClass('video-container self_vid'); 
	...
	getToken(sessionId).then(token => {
		...
		session.connect(token)
			.then(() => {
				...
				publisher = OV.initPublisher('selfvideo', {
...
function muteAudio() {
	...
	if (!audioEnabled) {
		...
		$('#mute-audio').removeClass('bg-dark');
		$('#mute-audio').addClass('showDiv');
	} else {
        $("#vol-btn").toggleClass('fas fa-microphone ');
        $("#vol-btn").toggleClass('fas fa-microphone-slash');
		$('#mute-audio').addClass('bg-dark');
		$('#mute-audio').removeClass('showDiv');
	}
...
function muteVideo() {
	...
	if (!videoEnabled) {
		...
		$('#mute-video').removeClass('bg-dark');
		$('#mute-video').addClass('showDiv');
	} else {
        $("#vid-btn").toggleClass('fas fa-video ');
        $("#vid-btn").toggleClass('fas fa-video-slash');
		$('#mute-video').addClass('bg-dark');
		$('#mute-video').removeClass('showDiv');
	}
...
var OPENVIDU_SERVER_SECRET = "Raghavendra1234!";
